                        ...
                        for it=1:t
                            for iz=1:z
                                for ic=1:nChannels
                                    ...
                                    if pretreatment
                                        IF(:,:,iz,ic,it)=IF(:,:,iz,ic,it)./Ifilter(:,:,ic);
                                    end
                                    counter = counter+1;
                                end
                            end
                        end
                        imp = copytoImagePlus(IF,'YXZCT');
                        imp.show();
                        ij.IJ.saveAs("Tiff",strcat(savefolder,'\',outputfilenames(iFile,ix,iy)));
                        imp.close();
                    end
                end
